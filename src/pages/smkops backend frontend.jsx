      ...
      fetchRecipes({
        ...
        .then(data => {
          ...
          const transformedRecipes = data.recipes.map((recipe = {}) => {
            ...
            const cuisinePath = (recipe.cuisinePath || '').toString()
            const category = cuisinePath
              ? cuisinePath.split('/').filter(p => p)[0] || 'Uncategorized'
              : 'Uncategorized'
            
            // Parse ingredients to create tags
            const ingredientsRaw = (recipe.ingredients || '').toString()
            const ingredientsList = ingredientsRaw
              ? ingredientsRaw.split(',').slice(0, 5).map(i => i.trim()).filter(Boolean)
              : []
            
            // Determine difficulty based on prep time
            const prep = Number(recipe.prepTime || 0)
            const cook = Number(recipe.cookTime || 0)
            const totalTime = recipe.totalTime || `${prep + cook} mins`
            let difficulty = "Easy"
            if (prep + cook > 60) difficulty = "Medium"
            if (prep + cook > 90) difficulty = "Hard"

            return {
              id: (recipe.id ?? '').toString() || `${Math.random()}`,
